#!/usr/bin/env bash # setup_and_run.sh
set -euo pipefail # Exit on error, undefined var, or pipe failure
# --------------------------------------------------------------------
# Script to set up environment, download data, and run analysis pipelines
# --------------------------------------------------------------------
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"  # repo root dir
ENV_NAME="advanced-statistical-learning" # name of conda/mamba env
PY_VER="3.10"                     # Python version
# --------------------------------------------------------------------
echo "==> Project root: ${REPO_ROOT}" # repo root dir
cd "${REPO_ROOT}" # cd to repo root
# Function to check if a command exists
have_cmd () { command -v "$1" >/dev/null 2>&1; } # check command existence
USE_MAMBA=0 # flag: use mamba if available
if have_cmd mamba; then # mamba is preferred
  USE_MAMBA=1
